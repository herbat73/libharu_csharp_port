// <auto-generated />
// Generated from src/hpdf_encoder.c.
namespace LibHaru.Internal;

internal static class SingleByteEncodingData
{
    internal static bool TryGetMap(string name, out char[] map)
    {
        if (Maps.TryGetValue(name, out var cached))
        {
            map = cached.ToArray();
            return true;
        }

        if (string.Equals(name, "ISO8859-1", StringComparison.Ordinal))
        {
            map = Latin1Map();
            return true;
        }
    ...
    private static char[] Latin1Map()
    {
        var map = new char[256];
        for (var i = 32; i < map.Length; i++)
            map[i] = (char)i;
        return map;
    }

    private static char[] CreateMap(int[] values)
    {
        var map = new char[256];
        for (var i = 0; i < values.Length; i++)
            map[i + 32] = (char)values[i];
        return map;
    }

    private static readonly Dictionary<string, char[]> Maps = new(StringComparer.Ordinal)
    {
        ["FontSpecific"] = CreateMap(
            [
