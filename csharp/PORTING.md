...
Status: the checked-in C headers, source modules, bundled fixtures, and canonical PDF demos have managed coverage. Remaining work is fixture-, tool-, or upstream-driven and tracked in the next section.
...
| --- | --- | --- |
| Core objects, dictionary, array, primitive values | `csharp/LibHaru/Internal` | Migrated for the local object model, including typed subclass validation for migrated wrappers. |
...
| TrueType/OpenType fonts | `TrueTypeFontLoader`, `PdfFontProgram`, `PdfDocument` | Migrated for TrueType simple/Type0 usage, TTC selection, embedding checks, dense subsetting, composite remapping, ToUnicode output, Type0 vertical `/DW2` metrics, simple OpenType/CFF `/FontFile3` embedding, and checked-in real-font fixture coverage. |
...
| U3D, 3D measure, shadings, ext graphics state | `PdfU3D`, `Pdf3DView`, `PdfShading`, `PdfExtGState`, `PdfPage` | Migrated for U3D streams, 3D views/nodes/camera/cross-sections, C3D/PD3 measures, ExData, axial/radial/free-form triangle mesh shadings, extended graphics state, and typed 3D validators. |
