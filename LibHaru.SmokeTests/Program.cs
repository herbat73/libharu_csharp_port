...
ReferenceOutputRegression.TryCompareExactPdfs(
    root.FullName,
    artifacts,
    Path.Combine(root.FullName, "LibHaru.SmokeTests", "fixtures", "reference-output.tsv"));
...
VisualRegression.TryRenderSmokePdfs(
    artifacts,
    Path.Combine(root.FullName, "LibHaru.SmokeTests", "fixtures", "visual-reference.tsv"),
    pdfPath,
    compressionPdfPath,
    fontPdfPath,
...
static void SmokeTest(string pdfPath)
{
    ...
    Require(HPDF_GetViewerPreference(pdf).HasFlag(PdfViewerPreference.HideToolbar),
        "Viewer preference getter mismatch.");
    Require(HPDF_GetInfoAttr(pdf, PdfInfoType.Title) == "Managed libharu smoke test",
        "Info attribute getter mismatch.");
    Require(
        HPDF_GetInfoAttr(pdf, PdfInfoType.ModDate)?.StartsWith("D:20260527123456", StringComparison.Ordinal) == true,
        "Info date setter mismatch.");
    ...
    using (var scratch = HPDF_New())
    {
        ...
        HPDF_NewDoc(scratch);
        Require(HPDF_HasDoc(scratch) && scratch.Pages.Count == 0 && scratch.PagePerPages == 0,
            "NewDoc should rebuild an empty active document.");
    ...
    Require(ReferenceEquals(winAnsiEncoder, HPDF_GetEncoder(pdf, "WinAnsiEncoding")),
        "Encoder lookup should be cached.");
    Require(HPDF_Encoder_GetType(winAnsiEncoder) == PdfEncoderType.SingleByte,
        "WinAnsi should report single-byte encoding.");
    Require(HPDF_Encoder_GetByteType(winAnsiEncoder, "ABC", 1) == PdfByteType.Single,
        "Single-byte encoder byte type mismatch.");
    ...
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "StandardEncoding"), 0x27) == 0x2019,
        "StandardEncoding quote mapping mismatch.");
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "MacRomanEncoding"), 0xDB) == 0x20AC,
        "MacRoman euro mapping mismatch.");
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "ISO8859-5"), 0xB0) == 0x0410,
        "ISO8859-5 Cyrillic mapping mismatch.");
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "CP1251"), 0xC0) == 0x0410,
        "CP1251 Cyrillic mapping mismatch.");
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "CP1258"), 0xD5) == 0x01A0,
        "CP1258 Vietnamese mapping mismatch.");
    Require(HPDF_Encoder_GetUnicode(HPDF_GetEncoder(pdf, "KOI8-R"), 0xE1) == 0x0410,
        "KOI8-R Cyrillic mapping mismatch.");

    var shiftJisVertical = HPDF_GetEncoder(pdf, "90ms-RKSJ-V");
    var shiftJisBytes = Encoding.Latin1.GetString([0x82, 0xA0]);
    Require(HPDF_Encoder_GetType(shiftJisVertical) == PdfEncoderType.DoubleByte,
        "CMap encoder should report double-byte encoding.");
    Require(HPDF_Encoder_GetByteType(shiftJisVertical, shiftJisBytes, 0) == PdfByteType.Lead,
        "CMap lead byte detection mismatch.");
    Require(HPDF_Encoder_GetByteType(shiftJisVertical, shiftJisBytes, 1) == PdfByteType.Trail,
        "CMap trail byte detection mismatch.");
    Require(HPDF_Encoder_GetByteType(shiftJisVertical, Encoding.Latin1.GetString([0x82]), 0) == PdfByteType.Lead,
        "CMap orphan lead-byte classification mismatch.");
    Require(
        HPDF_Encoder_GetByteType(shiftJisVertical, Encoding.Latin1.GetString([0x82, 0x20]), 1) == PdfByteType.Unknown,
        "CMap invalid trail-byte classification mismatch.");
    Require(HPDF_Encoder_GetUnicode(shiftJisVertical, 0x82A0) == 0x3042, "Shift-JIS Unicode map mismatch.");
    Require(HPDF_Encoder_GetWritingMode(shiftJisVertical) == PdfWritingMode.Vertical,
        "Vertical CMap writing mode mismatch.");
    var eucJapanese = HPDF_GetEncoder(pdf, "EUC-H");
    var eucJapaneseBytes = Encoding.Latin1.GetString([0xA4, 0xA2]);
    Require(HPDF_Encoder_GetByteType(eucJapanese, eucJapaneseBytes, 0) == PdfByteType.Lead,
        "EUC-JP lead byte detection mismatch.");
    Require(HPDF_Encoder_GetByteType(eucJapanese, eucJapaneseBytes, 1) == PdfByteType.Trail,
        "EUC-JP trail byte detection mismatch.");
    ...
    Require(HPDF_Encoder_GetType(HPDF_GetEncoder(pdf, "KSCms-UHC-H")) == PdfEncoderType.DoubleByte,
        "Korean CMap encoder type mismatch.");
    Require(HPDF_Encoder_GetWritingMode(HPDF_GetEncoder(pdf, "KSCms-UHC-HW-V")) == PdfWritingMode.Vertical,
        "Korean vertical CMap writing mode mismatch.");
    Require(HPDF_Encoder_GetType(HPDF_GetEncoder(pdf, "GBK-EUC-H")) == PdfEncoderType.DoubleByte,
        "Simplified Chinese CMap encoder type mismatch.");
    Require(HPDF_Encoder_GetWritingMode(HPDF_GetEncoder(pdf, "ETen-B5-V")) == PdfWritingMode.Vertical,
        "Traditional Chinese vertical CMap writing mode mismatch.");
    Require(HPDF_Encoder_GetType(HPDF_GetEncoder(pdf, "UTF-8")) == PdfEncoderType.DoubleByte,
        "UTF-8 encoder type mismatch.");
    Require(
        HPDF_Encoder_GetByteType(HPDF_GetEncoder(pdf, "KSCms-UHC-H"), Encoding.Latin1.GetString([0x81, 0x40]), 1) ==
        PdfByteType.Unknown, "KSCms-UHC trail byte range mismatch.");
    Require(
        HPDF_Encoder_GetByteType(HPDF_GetEncoder(pdf, "GB-EUC-H"), Encoding.Latin1.GetString([0xA1, 0xA0]), 1) ==
        PdfByteType.Unknown, "GB-EUC trail byte range mismatch.");
    ...
    var ttFontDef = HPDF_GetTTFontDefFromFile(pdf,
        Path.Combine(FindRepoRoot().FullName, "demo", "ttfont", "PenguinAttack.ttf"), true);
    ...
    Require(HPDF_Font_GetEncodingName(zapfDingbats) == "FontSpecific",
        "ZapfDingbats should default to FontSpecific encoding.");
    ...
    Require(fontWidth.NumChars == 7 && fontWidth.NumSpace == 1 && fontWidth.Width > 0,
        "Font text-width alias mismatch.");
    ...
    var imageForm = HPDF_Page_CreateXObjectFromImage(pdf, page, new PdfRect(0, 0, 16, 16), rawImage, true);
    var whiteForm = HPDF_Page_CreateXObjectAsWhiteRect(pdf, page, new PdfRect(0, 0, 12, 10));
    Require(HPDF_Page_GetXObjectName(page, imageForm).StartsWith("X", StringComparison.Ordinal),
        "Image form XObject name mismatch.");
    Require(HPDF_Page_GetXObjectName(page, whiteForm).StartsWith("X", StringComparison.Ordinal),
        "White form XObject name mismatch.");
    ...
    Require(dash.Count == 2 && dash.Pattern[0] == 2.0 && dash.Pattern[1] == 4.0 && dash.Phase == 1,
        "Dash getter mismatch.");
    ...
    Require(HPDF_Page_GetTransMatrix(page) == new PdfTransMatrix(1, 0, 0, 1, 5, 6),
        "Transformation matrix getter mismatch.");
    HPDF_Page_GRestore(page);
    Require(HPDF_Page_GetGStateDepth(page) == 1 && HPDF_Page_GetLineWidth(page) == 1,
        "Graphics restore did not restore state.");
    ...
    Require(HPDF_Page_GetWordSpace(page) == 1.5 && HPDF_Page_GetCharSpace(page) == 0.5,
        "ShowTextNextLineEx spacing mismatch.");
    ...
    HPDF_Page_SetFontAndSize(page, font, 10);
    HPDF_Page_BeginText(page);
    var textRectStatus = HPDF_Page_TextRect(page, 72, 180, 260, 130, "TextRect alias wraps this sentence.",
        PdfTextAlignment.Left, out var textRectLength);
    HPDF_Page_EndText(page);
    ...
    var markup = HPDF_Page_CreateTextMarkupAnnot(page, new PdfRect(72, 188, 220, 202), "generic markup", null,
        PdfAnnotType.Highlight);
    HPDF_TextMarkupAnnot_SetQuadPoints(markup, new PdfPoint(72, 188), new PdfPoint(220, 188), new PdfPoint(220, 202),
        new PdfPoint(72, 202));
    ...
    Require(latin1.Contains("/BaseFont /SimSun", StringComparison.Ordinal),
        "Missing Simplified Chinese CID font object.");
    Require(latin1.Contains("/BaseFont /MingLiU", StringComparison.Ordinal),
        "Missing Traditional Chinese CID font object.");
    ...
    Require(latin1.Contains("/MissingWidth 500", StringComparison.Ordinal),
        "Missing predefined CID descriptor missing width.");
    Require(latin1.Contains("/FontBBox [-121 -136 996 859]", StringComparison.Ordinal),
        "Missing exact MS-PGothic font bbox.");
