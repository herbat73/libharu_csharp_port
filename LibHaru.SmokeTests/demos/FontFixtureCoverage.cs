...
public static class FontFixtureCoverage
{
    ...
    public static void Test(string repoRoot, string artifactsDir)
    {
        ...
        foreach (var fixture in fixtures)
            TestFixtureFont(fixture,
                Path.Combine(outputDir, $"{SafeName(Path.GetFileNameWithoutExtension(fixture.FontPath))}.pdf"));
    ...
    private static void TestFixtureFont(FontFixture fixture, string pdfPath)
    {
        ...
        var fontName = string.Equals(Path.GetExtension(fixture.FontPath), ".ttc", StringComparison.OrdinalIgnoreCase)
            ? HPDF_LoadTTFontFromFile2(pdf, fixture.FontPath, 0, true)
            : HPDF_LoadTTFontFromFile(pdf, fixture.FontPath, true);

        Require(fontName == fixture.ExpectedBaseFont,
            $"{fixture.FontPath} loaded as {fontName}; expected {fixture.ExpectedBaseFont}.");

        var page = HPDF_AddPage(pdf);
        if (fixture.ExpectsCidKeyedCff)
        {
            Require(flavor == "OTTO",
                $"{fixture.FontPath} manifest marks a CID-keyed CFF fixture but the SFNT flavor is {flavor}.");
            ...
            Require(
                verticalWidth.NumChars == verticalText.Length &&
                verticalWidth.Width == verticalAdvance * verticalText.Length,
                $"{fixture.FontPath} did not use vertical DW2 displacement metrics.");
        ...
        else
        {
            ...
            if (flavor != "OTTO")
            {
                ...
                Require(verticalWidth.NumChars == 2 && verticalWidth.Width == verticalAdvance * 2,
                    $"{fixture.FontPath} did not use vertical DW2 displacement metrics.");
        ...
        Require(latin1.Contains($"/BaseFont /{fontName}", StringComparison.Ordinal),
            $"{fixture.FontPath} did not write the expected BaseFont.");
        Require(latin1.Contains("/FontDescriptor", StringComparison.Ordinal),
            $"{fixture.FontPath} did not write a FontDescriptor.");

        if (fixture.ExpectsCidKeyedCff)
        {
            Require(latin1.StartsWith("%PDF-1.6", StringComparison.Ordinal),
                $"{fixture.FontPath} did not raise the PDF version for OpenType embedding.");
            Require(latin1.Contains("/Subtype /Type0", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a Type0 font dictionary.");
            Require(latin1.Contains("/Subtype /CIDFontType0", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a CIDFontType0 descendant.");
            Require(latin1.Contains("/Encoding /Identity-H", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write Identity-H encoding.");
            Require(latin1.Contains("/Encoding /Identity-V", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write Identity-V encoding.");
            Require(latin1.Contains("/ToUnicode", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a ToUnicode CMap.");
            Require(latin1.Contains("/DW2 [", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write vertical DW2 metrics.");
            Require(latin1.Contains("/W [", StringComparison.Ordinal), $"{fixture.FontPath} did not write CID widths.");
            Require(latin1.Contains("/FontFile3 ", StringComparison.Ordinal),
                $"{fixture.FontPath} did not embed an OpenType FontFile3 stream.");
            Require(latin1.Contains("/Subtype /OpenType", StringComparison.Ordinal),
                $"{fixture.FontPath} did not identify the embedded OpenType program.");
            Require(!latin1.Contains("/CIDToGIDMap", StringComparison.Ordinal),
                $"{fixture.FontPath} should not write a CIDToGIDMap for CIDFontType0.");
        }
        else if (flavor == "OTTO")
        {
            Require(latin1.StartsWith("%PDF-1.6", StringComparison.Ordinal),
                $"{fixture.FontPath} did not raise the PDF version for OpenType embedding.");
            Require(latin1.Contains("/Subtype /Type1", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a CFF-backed Type1 font dictionary.");
            Require(latin1.Contains("/FontFile3 ", StringComparison.Ordinal),
                $"{fixture.FontPath} did not embed an OpenType FontFile3 stream.");
            Require(latin1.Contains("/Subtype /OpenType", StringComparison.Ordinal),
                $"{fixture.FontPath} did not identify the embedded OpenType program.");
        ...
        {
            Require(latin1.Contains("/Subtype /TrueType", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a TrueType font dictionary.");
            Require(latin1.Contains("/Subtype /Type0", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a Type0 font dictionary.");
            Require(latin1.Contains("/Subtype /CIDFontType2", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a CIDFontType2 descendant.");
            Require(latin1.Contains("/Encoding /Identity-H", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write Identity-H encoding.");
            Require(latin1.Contains("/Encoding /Identity-V", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write Identity-V encoding.");
            Require(latin1.Contains("/ToUnicode", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a ToUnicode CMap.");
            Require(latin1.Contains("/CIDToGIDMap ", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write a CIDToGIDMap stream.");
            Require(latin1.Contains("/DW2 [", StringComparison.Ordinal),
                $"{fixture.FontPath} did not write vertical DW2 metrics.");
            Require(latin1.Contains("/FontFile2 ", StringComparison.Ordinal),
                $"{fixture.FontPath} did not embed a FontFile2 stream.");
            var embeddedLength = FindFontFile2Length1(latin1);
            var sourceLength = new FileInfo(fixture.FontPath).Length;
            Require(embeddedLength > 0 && embeddedLength < sourceLength,
                $"{fixture.FontPath} embedded the full font instead of a subset.");
        }
    }

    private static void ValidateLoadedFont(FontFixture fixture, PdfFont font, char probe)
    {
        Require(!string.IsNullOrWhiteSpace(HPDF_Font_GetFontName(font)),
            $"{fixture.FontPath} did not expose a font name.");
        Require(HPDF_Font_GetFontName(font) == fixture.ExpectedBaseFont,
            $"{fixture.FontPath} returned an unexpected font handle name.");
        Require(HPDF_Font_GetUnicodeWidth(font, probe) > 0,
            $"{fixture.FontPath} did not expose width data for U+{(int)probe:X4}.");
        Require(HPDF_Font_GetAscent(font) > 0, $"{fixture.FontPath} did not expose ascent metrics.");
        Require(HPDF_Font_GetBBox(font).Right > HPDF_Font_GetBBox(font).Left,
            $"{fixture.FontPath} did not expose a valid bounding box.");
    }
    ...
    private static FontFixture LoadFixture(string fontPath)
    {
        ...
        Require(formats.Contains(extension, StringComparer.OrdinalIgnoreCase),
            $"Font fixture manifest does not list {extension}: {manifestPath}.");
        Require(license.Contains("Open Font License", StringComparison.OrdinalIgnoreCase),
            $"Font fixture manifest should record the open font license: {manifestPath}.");
    ...
    private static string ReadRequiredString(JsonElement root, string propertyName, string manifestPath)
    {
        ...
        if (string.IsNullOrWhiteSpace(value))
            throw new InvalidOperationException(
                $"Font fixture manifest has an empty '{propertyName}': {manifestPath}.");
