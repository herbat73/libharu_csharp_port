...
public static class PdfAValidation
{
    public static void Test(string repoRoot)
    {
        ...
        using (var namedJavaScript = HPDF_New())
        {
            ...
            HPDF_AddNamedJavaScript(namedJavaScript, "boot",
                HPDF_CreateJavaScript(namedJavaScript, "app.alert('nope');"));
        ...
        using (var threeDAnnotation = HPDF_New())
        {
            ...
            HPDF_Page_Create3DAnnot(page, new PdfRect(10, 10, 80, 80),
                HPDF_LoadU3DFromMem(threeDAnnotation, [1, 2, 3, 4]));
        ...
        using (var pdfA4 = HPDF_New())
        {
            ...
            Require(latin1.StartsWith("%PDF-2.0", StringComparison.Ordinal),
                "PDF/A-4 should bump the PDF header to 2.0.");
            Require(latin1.Contains("<pdfaid:rev>2020</pdfaid:rev>", StringComparison.Ordinal),
                "PDF/A-4 XMP should include pdfaid:rev 2020.");
        }
        ...
        using (var embeddedInPdfA3 = HPDF_New())
        {
            ...
            Require(latin1.Contains("/AFRelationship /Unspecified", StringComparison.Ordinal),
                "PDF/A-3 embedded file should get a default AFRelationship.");
            Require(latin1.Contains("/AF [", StringComparison.Ordinal),
                "PDF/A-3 embedded file should be associated from the catalog.");
        }
        ...
        foreach (var (type, header, part, conformance, requiresRevision) in PdfAIdentificationCases())
        {
            ...
            Require(latin1.Contains($"<pdfaid:part>{part}</pdfaid:part>", StringComparison.Ordinal),
                $"{type} XMP part mismatch.");
            Require(
                latin1.Contains($"<pdfaid:conformance>{conformance}</pdfaid:conformance>", StringComparison.Ordinal),
                $"{type} XMP conformance mismatch.");

            if (requiresRevision)
                Require(latin1.Contains("<pdfaid:rev>2020</pdfaid:rev>", StringComparison.Ordinal),
                    $"{type} should include pdfaid:rev 2020.");
            else
                Require(!latin1.Contains("<pdfaid:rev>", StringComparison.Ordinal),
                    $"{type} should not include a PDF/A revision.");
        }
    ...
    private static (PdfPdfAType Type, string Header, string Part, string Conformance, bool RequiresRevision)[]
        PdfAIdentificationCases()
    {
        return
        [
